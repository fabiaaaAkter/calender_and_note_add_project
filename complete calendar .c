...
void showNote(int mm){
    ...
    while(fread(&R,sizeof(R),1,fp) == 1){
        if(R.mm == mm){
            gotoxy(10,5+i);
            printf("Note %d Day = %d: %s", i, R.dd,  R.note);
            isFound = 1;
            i++;
        }
    }
...
int main(){
    ...
    SetConsoleTitle("Calender/Note taking Project");
    int choice;
    while(TRUE){
        system("cls");
        printf("             welcome to digital calender\n");
        printf("1. Find Out the Day\n");
        printf("2. show all the day of the month\n");
        printf("3. Add Note\n");
